              ...
              return (
                <button
                  ...
                  className={`relative overflow-hidden rounded-lg p-4 text-left transition-all duration-300 ${
                    isSelected ? 'bg-raised border-2 border-primary/50' : 'bg-surface/50 border-2 border-border hover:border-outline'
                  }`}
                >
                  {isSelected && <div className={`absolute inset-0 opacity-10 bg-gradient-to-br ${dept.color}`} />}
                  <div className="relative z-10">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-2 ${isSelected ? `bg-gradient-to-br ${dept.color}` : 'bg-raised'}`}>
