...
const AnimatedGradientBackground = memo(function AnimatedGradientBackground() {
  ...
  return (
    <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none">
      ...
      {/* Coral accent orb */}
      <motion.div
        className="absolute w-[350px] h-[350px] rounded-full opacity-10 blur-[60px]"
        style={{
          background: 'radial-gradient(circle, rgba(244,98,42,0.35) 0%, transparent 70%)',
          willChange: 'transform',
        }}
        animate={{
          x: prefersReducedMotion ? 0 : [0, 80, -60, 0],
          y: prefersReducedMotion ? 0 : [0, -50, 70, 0],
          scale: prefersReducedMotion ? 1 : [1, 1.05, 0.9, 1],
        }}
        transition={{
          duration: 28,
          repeat: Infinity,
      ...
      {/* Dot grid overlay */}
      <div className="absolute inset-0 geo-grid-dots opacity-40" />
